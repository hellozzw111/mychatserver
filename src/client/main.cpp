...
void doLoginResponse(json &responsejs)
{
    ...
    else // 登录成功
    {
        ...
        {
            {
                lock_guard<mutex> lock(_friendsMutex);
                // 初始化
...
vector<User> getFriends(int id){
    fixbug::SearchServiceRpc_Stub stub(new MprpcChannel());
    //rpc请求的参数
    fixbug::SearchUsersRequest request;
    request.set_id(id);
    // rpc方法的响应消息
    fixbug::SearchUsersResponse response;
    //发起rpc请求的方法调用  同步的rpc调用过程 MprpcChannel::callmethod()
    stub.SearchUsers(nullptr, &request, &response, nullptr);

    //一次调用结果的接受
    if (0 == response.result().errcode())
    {
        //处理请求成果得到的vector<User> users结果
        {
            lock_guard<mutex> _friendlock(_friendsMutex);
            g_currentUserFriendList.clear();
            for(int i=0;i<response.users_list_size();i++){
                auto temp = response.users_list(i);
                User temp_user;
                temp_user.setId(temp.id());
                temp_user.setName(temp.name());
                temp_user.setState(temp.state());
                g_currentUserFriendList.emplace_back(temp_user);
            }
        }
    }
    else
