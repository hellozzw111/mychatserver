...
ServerManager::ServerManager() {
    stub = new dataserver::DataService_Stub(new MprpcChannel());
}
...
void ServerManager::doAdd(string file_name, Node* parent, string contents) {
    dataserver::AddRequest request;
    request.set_file_name(file_name);
    request.set_server_name(parent->file_name);
    request.set_contents(contents);
    dataserver::AddResponse reponse;
    MprpcController controller;
    stub->Add(&controller, &request, &reponse, nullptr);
    if(!reponse.success()) {
...
string ServerManager::doGet(string file_name, Node* parent) {
    dataserver::GetRequest request;
...
void ServerManager::doDel(string file_name, Node* parent) {
    dataserver::DelRequest request;
    dataserver::DelResponse response;
    request.set_file_name(file_name);
    request.set_server_name(parent->file_name);
    MprpcController controller;
    stub->Del(&controller, &request, &response, nullptr);
    if(!response.success()) {
